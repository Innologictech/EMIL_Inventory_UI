...
export class RightsidebarComponent implements OnInit {
  ...
  document.documentElement.setAttribute('data-layout', 'horizontal');
    ...
    this.store.dispatch(changeSidebarMode({ sidebarMode: 'hidden' }));
  document.documentElement.setAttribute('data-sidebar', 'hidden');
   ...
   const vertical = document.getElementById('is-layout');
  if (vertical) {
    vertical.removeAttribute('checked'); // force uncheck vertical
  }
  }
  ...
  this.store.select(getLayoutMode).subscribe((layout) => {
    ...
    // Force refresh the menu display
    setTimeout(() => {
...
updateMenuVisibility() {
  // Add a class to body that will show menu labels
