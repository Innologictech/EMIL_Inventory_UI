...
export class AuthenticationEffects {
  ...
  Register$ = createEffect(() =>
    this.actions$.pipe(
      ofType(Register),
      exhaustMap(({ employeeCode, username, password }) => {
        if (environment.defaultauth === 'fakebackend') {
          return this.userService.register({ employeeCode, username, password }).pipe(
            map((user) => {
          ...
          return this.AuthenticationService.register({ employeeCode, username, password }).pipe(
            map((user) => {
              this.router.navigate(['/auth/login']);
              return RegisterSuccess({ user })
            })
  ...
  login$ = createEffect(() =>
    this.actions$.pipe(
      ofType(login),
      exhaustMap(({ employeeCode, password }) => {
        if (environment.defaultauth === "fakebackend") {
          return this.AuthfakeService.login(employeeCode, password).pipe(
            map((user) => {
              if (user) {
          ...
          return this.AuthenticationService.login(employeeCode, password).pipe(map((user) => {
            return loginSuccess({ user });
          }))
        }
      })
